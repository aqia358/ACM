...
public class ListToBST2 {
	...
	public static TreeNode sortedListToBST(ListNode head){
		...
		return buildTree(head, 1, size, list);
	}
	
	public static TreeNode buildTree(ListNode head, int s, int e, List<ListNode> list){
		if(s > e) return null;
		int m = (s + e)/2;
		ListNode temp = list.get(m - 1);
		TreeNode root = new TreeNode(temp.val);
		root.left = buildTree(head, s, m - 1, list);
		root.right = buildTree(head, m + 1, e, list);
		return root;
	}
